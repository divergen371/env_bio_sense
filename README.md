...
- **オンデマンドWi-Fi APモード（ファイルリモートアクセス）**:
  - 稼働中に `BOOT` ボタンを3秒間長押しすることで、ESP32自身がアクセスポイント（AP）となり、スマホやPCから直接内部のSDカードへアクセスできます（`http://env.local/` または `192.168.4.1`）。
  - **重要**: 巨大なファイル送信時のSPIバス競合（クラッシュ）を防ぐため、Wi-FiがONになっている間はSDカードへの新規データの書き出しを一時停止します。
  - 停止中のデータはすべてFRAMにバッファリングされ、Wi-FiをOFFにすると一括でSDに書き出されて復旧しますが、**Wi-FiをONにしたまま約37分（バッファ上限）を超えると古いデータから上書きされ欠損します**。Wi-Fiはファイルのダウンロード時のみ一時的にONにして使用してください。
