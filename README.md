...
- **自作脈波DSPエンジン (`PulseAnalyzer`)**:
  - MAX30102向け独自時間領域分析アルゴリズム。
  - **ゼロクロス・ダウン再武装**: 巨大アーティファクトによる永久ロックアップを数学的に防止。
  - **DC高速初期化**: 指設置後約2秒でベースラインを収束させ、約10秒で安定計測を開始。
...
- **ネイティブ単体テスト基盤 (`pio test -e native`)**:
  - Unityフレームワークおよび25Hz実効サンプリングレートに適合させた自動単体テスト環境。
...
- `utils/pulse_analyzer.cpp`: 自作脈波解析エンジン（DC除去、ローパス、ゼロクロス再武装ヒステリシスピーク検出、Maxim公式SpO2多項式）
